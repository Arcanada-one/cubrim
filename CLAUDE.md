...
11. **A strong rANS/BWT entropy backend SUBSUMES simple pre-transforms (delta-of-delta, dictionary→RLE, MTF) that win only in a bit-packing context with no entropy coder — spike the transform THROUGH the real backend, never against a bit-packed strawman. **H-49/H-49-reborn extends this to cross-column: cross-stream mutual information is non-subsumed by the BYTE model but NEARLY-subsumed by the per-column ENTROPY coder — measured on BOTH temporal telemetry (forex 0.998x) AND the ideal non-temporal class (covtype 40 mutually-exclusive one-hots 1.020x; adult exact bijection 1.052x). rANS already compresses each correlated/sparse/redundant column near its entropy, so explicit collapse recovers only the independent-vs-joint gap (~1.44 bits/row for one-hots; the whole redundant column for a bijection) — 2-5% of the compressed file, because redundant columns are low-entropy and already compress to ~nothing. A whole-file ≥1.5x needs the redundancy to dominate the FILE, which low-entropy columns never do. Corra/ALP −53..−85% literature wins are vs a WEAK per-column baseline (Parquet dict/bit-pack), not a strong entropy coder; against rANS the gap collapses. Likewise H-50: ALP-RD's 'low mantissa is random → bitpack raw' loses on low-precision real doubles (UCI ~7-digit) where the byte backend already entropy-codes that structure (sub-byte separation 0.40-0.59x vs zstd).**** Gorilla DoubleDelta, Parquet dict+RLE, and bzip2 MTF are real wins *because their backends bit-pack* (storing a repeated constant costs bits/value). Cubrim already entropy-codes with rANS/geomix after BWT, which crushes a constant/low-cardinality stream to ~0 — so the extra transform adds nothing and often *hurts* (it perturbs the column-major stream and raises entropy). Measured: (H-41) on a fixed-interval timestamp column (delta stddev/mean = 0.00, DoubleDelta's best case) delta-of-delta was +6.8 %/+12.9 % WORSE than single-delta through the cubrim rail; the single-delta path already wins the fixed-interval metric class −44 %/−75 % vs zstd. (H-48) dict→RLE on a maximally enum-heavy run-structured CSV gained only −2.3 % over the existing BWT+geomix columnar path (already −52 % vs zstd), i.e. mostly subsumed, ~0 on real numeric-heavy telemetry. Lesson: for a delta-order / RLE / MTF idea, the win must clear the bar *after* the entropy backend, on a faithful spike; the structural levers that DO transfer are the ones that change the INFORMATION (columnar reorder so a column's values cluster — H-30; integer/decimal delta that shrinks the value magnitude — H-31/H-40), not the ones that merely re-encode what the entropy coder already handles. The recurring tell: «this transform wins for Gorilla/Parquet/bzip2» is necessary but NOT sufficient — those tools lack Cubrim's entropy stage.

12. **A frozen identity must pin its INPUTS, not its OUTPUTS — and an identity check bound to a MUTABLE external input (live `origin/main`, the live crates.io index) can never pass again once that input moves. Two independent lanes died of this exact class.** (a) **NEW-24 G6 prebuild**: the protocol froze a generated `Cargo.lock` SHA-256, but `Cargo.lock` is gitignored and every dependency in `code/cubrim-rs/Cargo.toml` is a semver-compatible range, so `cargo generate-lockfile` re-resolved against the live crates.io index at run time. The constant was captured 2026-08-09T16:57:54Z; `futures-core`/`futures-task`/`futures-util` 0.3.34 were published 2026-08-11T12:13Z, ~1h before the run, and the lock hash moved `0080335e…` → `0d17c1fc…`. Both independent clones still produced BYTE-IDENTICAL locks, so the double-source-independence property held perfectly — only the frozen constant was unreproducible, and the failure consumed a one-shot allowance that can never be retried. It survived G2→G4 only because those ran within hours of capture. (b) **NEW-02 historical validation**: the capture harness guards `rev-parse HEAD == code_sha AND rev-parse origin/main == code_sha` (`new02_oracle_grid.py:393-396`). That is CORRECT at capture time and WRONG when the same function is reused at validation time — once main advances past the execution commit `708cda9`, the derived package's verifier fails permanently (`code identity is not the actual exact origin/main SHA`), with 8 of its 9 tests otherwise green. Lesson: a capture-time guard is not a validation-time guard — separate them. For any frozen scientific identity, ask *what mutable thing must stay still for this to reproduce?* If the answer is a live registry, a moving branch ref, or wall-clock-dependent resolution, the identity is a time bomb: commit the exact input as a reviewed artefact (an exact `Cargo.lock` built `--locked`, a vendored registry, the frozen execution commit) rather than pinning a hash of what that input happens to produce today.

13. **A CI runner move needs THREE charged branches — capability, capacity, and co-tenancy — and proving the host CAN run the job says nothing about the other two. A runner LABEL is not a host.** Same "charge every branch" family as #6/#7/#8, applied to infrastructure. Measured (CUBR-0087, 2026-08-12, `cubrim-site` `test.yml` dead since 2026-08-09 because GitHub Actions billing blocks the job from starting): the capability probe was a clean GO — on the target host PHP 8.4.19/8.3.6 passes the job's own `version_compare(PHP_VERSION,"8.3",">=")` assertion, `npx playwright install chromium` succeeds **without** `--with-deps` (the runner user has no sudo, so the flag as written would fail), and the full suite ran **121 passed in 3.9m**. Two unmeasured branches then killed it. (a) **Capacity:** `arcana-ci-general` is a *single registered runner, not a pool*, so jobs serialise — checked live, five `auth-arcana` CI runs sat **queued, oldest 2h29m**, host at load 28–38 on 16 cores. The move would convert "red for billing" into "pending for hours", which on a repo without branch protection is a *weaker* signal, not a restored one. (b) **Co-tenancy:** the runner user is in the `docker` group (root-equivalent on that host) and the same runner executes an auth service's CI; a `pull_request` job runs arbitrary dependency install hooks, so a poisoned transitive npm package in a dependabot PR — no fork required — puts less-trusted code on non-ephemeral infrastructure shared with a security-sensitive pipeline. Separately, **labels do not identify hosts**: `ci-general` spans two hosts with *different* PHP (8.3.6 vs none), and `arcana-ai` names a runner installed on arcana-devs, not on dev-ai — resolve label→host from each runner's own `.runner` `agentName`, never from the label text, and check what the *existing* consumers pin (`gh search code --owner <org> "<label>"`) before assuming a slot is free. Lesson: for any "move the job to our own runners" proposal, run the capacity probe (queue depth on that exact label *right now*) and the co-tenancy probe (what else executes on that host, and does this job execute untrusted PR code) BEFORE the capability probe, because capability is the branch most likely to pass and least likely to decide. A GO from capability alone is unsound — I reached the wrong conclusion three times on this one question, and only an adversarial review caught it.
