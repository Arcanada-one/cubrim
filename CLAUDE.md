...
4. **BWT pre-pass on the value stream is the one GO lever — non-i-order serialization is what works.** A Burrows-Wheeler Transform of the value-code block before entropy coding builds its OWN locality (it does not sort by phi), giving the order-1 coder a stream it has never seen. Measured (CUBR-0020): order-1 conditional entropy drops +91.4% on `log_like`, +65.7% on `text`; aggregate 7-file ratio T4 0.587240 → T5(auto) 0.491196 = −16.36%, byte-exact lossless round-trip on all 7 files. Inverse needs ONLY the primary index (one LEB128 varint), NOT a stored suffix array — the CUBR-0017 research note's "~n·log n suffix-array overhead" claim was wrong. The selector (`ValueScheme::Auto`) picks BWT only on a strict per-file size win, so sparse/random files fall back losslessly (non-regression). This is why BWT succeeds where every geometry-side idea (#3/#5/#6) fails: it changes the entropy of the input, not just its geometry.
...
6. **Phi maps POSITION, not VALUE — every geometry-side alphabet/axis idea is inert.** The phi coordinate `x_k = (i // B^k) mod B` depends only on the input position `i`, never on the byte value at that position. Consequence: no choice of N, axis-traversal order, or per-axis partition can make the value-alphabet uneven across axes or change the i-order entropy the value coder sees. Three NO-GOs confirm this from different measurement angles — CUBR-0018 (axis-sort traversal: destroys runs), CUBR-0019 (N-dim sweep: H N-invariant), CUBR-0021 (per-axis alphabet cardinality: phi spreads the value-alphabet evenly; axis-0 stride-B sampling always sees the full alphabet, and a per-axis "narrow" width is non-invertible because a value lies on ALL axes at once, so its realisable width = max over axes = base width; the only non-zero realisable saving is on tiny-alphabet synthetic files already dominated by RleCodes/T4). Lesson: any hypothesis that needs phi to partition/correlate/narrow VALUES across axes is structurally blocked — predict NO-GO and confirm with the cheap ~50-LoC Python entropy/cardinality probe BEFORE any Rust. Only non-i-order serialization that builds its own locality (Gotcha #4, BWT) moves the ratio.
