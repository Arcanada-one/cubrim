...
## H-54-IMPL — MODE_BINFLOAT shipped: GO (binary float-array SoA + reversible delta)

- **STATUS:** GO, shipped (container mode 6). Greenlit after the H-54 spike GO; generalisation hardened on 5 real KITTI scans first (operator condition).
- **GENERALISATION (before Rust):** win is CLASS-BOUND, needs spatially-smooth row order. 5 real KITTI Velodyne scans (azureology+kuixu+darylclimb, tight 0.03–0.04 m order): **1.46/1.50/1.56/1.57/1.62× vs zstd-19 (mean 1.54, 4/5 ≥1.5), lossless RT byte-exact**. Cross-sensor: nuScenes LIDAR_TOP 1.03× (loose 0.53 m order, re-ordered demo), ScanNet 1.06×, SUN RGB-D 1.00× (indoor RGB-D, no scan order). Honest scope: Cubrim crushes zstd only on RAW spinning-LiDAR in native firing order; the delta lever is an ordering property of the input, not of "binary floats" generally.
- **BUILT:** MODE_BINFLOAT=6 (header.rs) + encode_binfloat/decode_binfloat. AoS→SoA at auto-picked record width (order-0 proxy over {12,16,20,24,28,32}); each column competitively raw or reversible wrapping-uint32 delta of the float bits (per-col mode flag); each column nested through full LZ/columnar/base competition (`encode_with_config_inner(try_binfloat=false)` — the LZ pass entropy-codes the delta streams; encode_base alone bitpacks raw). Activation gate len>64KB ∧ len%4==0 ∧ ≥75% plausible-float (PERF guard; ratio safety = competitive min).
- **REGRESSION-PROOF (measured):** first cut short-circuited LZ/columnar on binfloat-detect → REGRESSED SUN RGB-D 702039→739956 (LZ codes it better). Fixed by competing binfloat ALONGSIDE LZ/columnar via min(): SUN RGB-D now MODE_LZ 702039 (=pre-binfloat byte-exact); all 8 clouds ≤ pre-binfloat output.
...
## CUBR-REEVAL — GO hypotheses vs HONEST baselines (methodology re-validation)

- **TRIGGER:** operator+consilium adversarial audit — zstd-19 baseline = weak strawman; re-measure every GO vs domain codecs (LAZ/BGZF) + strongest universal (xz-9e/PPMd/brotli-q11). Re-measurement only, codec untouched, NOT pushed. Tools: codec-venv laspy2.7+lazrs, pysam0.24, xz, 7z PPMd, brotli.
- **H-54 LiDAR → RECLASSIFY (loses to domain LAZ as shipped):** 5 real KITTI scans, LAZ@1mm verified value-lossless (KITTI sensor ~2cm, ndiff=0 all 4 channels). AGGREGATE LAZ 2660589 vs Cubrim binfloat 3042697 = **LAZ wins 1.144×** (every scan −10..−14%). Cubrim DOES beat all GENERIC universal (kitti.bin: zstd 905645, xz 704996, ppmd 746163, brotli 812029 all > Cubrim 577217). SUBSUMPTION-FIX: H-54 transform(SoA+delta) + ppmd = 425736 **BEATS LAZ 495640 by 1.16×** → transform NOT subsumed; shipped codec loses ONLY because Cubrim backend (BWT+geomix/LZ) weaker than ppmd/xz on delta-of-float streams (consilium flaw#2 confirmed for THIS stream type). Honest: "1.54× vs zstd" was weak-baseline artifact for shipped-vs-domain; beats generic, loses to domain LAZ −14%; the lever is real, gated by backend.
- **H-52 VCF → HOLDS:** real 1000G chr20, Cubrim --value-scheme bwt-rans (default BitpackFixed=92332 was config-mismatch trap, caught before recording false regression; champion config reproduces 19931/39020 exactly). Cubrim s400=19931/s1000=39020 beats BGZF(domain .vcf.gz) **3.3-4.1×**, xz-9e **1.42-1.52×**, PPMd **1.49-1.68×**. PBWT = genomic-SOTA family (GTShark/GTC PBWT-based, not directly benched). Win survives honest baselines.
- **H-29/31/40 Telemetry → HOLDS:** real forex/telemetry CSVs, Cubrim bwt-rans. forex_tick 26848 / forex_usdchf 24881 / status_timeseries 11702 beat STRONGEST universal (xz/ppmd/brotli) by **−31..−46%** (was −53.6% vs zstd). Realized through Cubrim's OWN backend (strong enough on text-derived numeric streams, unlike LiDAR). Parquet domain codec not benched (pyarrow unavailable) — follow-up.
- **NET (honest):** Cubrim genuine structural winner vs strong universal on VCF + telemetry; on LiDAR beats generic but shipped backend loses to domain LAZ (transform does not). No GO fully retracted; LiDAR reclassified "beats generic, not domain codec". Lessons: (1) zstd strawman real for LiDAR only; (2) backend weakness real for LiDAR delta-of-float streams; (3) always pin --value-scheme to champion when re-measuring. Honesty over wins. Report: consilium/reports/CUBR-REEVAL-honest-baselines.md

---

## CUBR-BACKEND-SPIKE — PPMd-class entropy backend (Option A, consilium unanimous): GO-to-plan

- **STATUS:** spike GO (margin real, materialised by reference PPMd); integration = genuine multi-day PPMd, plan-don't-rush. No code, codec untouched, invariants intact, NOT pushed.
- **PREMISE:** Cubrim transforms best-in-class; bottleneck = downstream entropy backend (BWT+rANS/geomix, ~5/8 general). Strengthen it → lifts every class.
- **LiDAR float-delta (per-column kitti.bin):** ppmd6 beats cub bwt-rans 1.152×/column; transform+ppmd total 427933 < LAZ 495640 (1.16×). The margin the brief targets.
- **CONFIG-MISMATCH CORRECTION (3rd trap, important):** re-eval "LiDAR loses LAZ −14%" used DEFAULT BitpackFixed (577217). Champion bwt-rans: all 5 KITTI scans BEAT LAZ, AGG Cubrim 2589882 vs LAZ 2660589 = **1.027×**, RT byte-exact. LiDAR HOLDS vs domain LAZ; backend upgrade turns +2.7% into ~+16%.
- **GENERAL DATA (universal prize):** ppmd beats Cubrim-champion on EVERY holdout file — c_header 1.31×, config 1.32×, prose 1.18×, rust_src 1.15×, exe 1.11×, csv 1.08×. PPMd-class scheme → rank 5/8→2-3.
...
## CUBR-LEADER-GAP — literature resolution onto existing cards

- **NEW-09 + NEW-10 PE section-aware BCJ2:** split branch targets/control from ordinary bytes, charge PE section metadata, and beat the live 7z exe aggregate (`0.2748738268008853`). Extends the measured H-45 x86 BCJ win; RT=OK/cmp=0 and full-24 zero-regression are mandatory.
- **IW-05 SPARC BCJ:** correct the physical type of Canterbury `sum` (SPARC executable, currently grouped as binary) and test a matched SPARC converter against xz `0.24843`; wrong-architecture filters are controls.
...
## WAVE 3 (FH3-01..FH3-09) — свежие гипотезы, 2026-07-22 (Fable 5, research-only)

Третья волна. Контекст сместился: Cubrim теперь **#1/10 по ВСЕМ 6 типам + overall** (task CUBR-0064-full24-cm2-retained-min) — цель не «догнать», а **увеличить margin и застраховать лидерство**, особенно image (margin всего −4.6%, тоньше всех), затем exe (−9.7%), database (−9.8%). Анти-дупликация: прочитаны hypothesis-log (H-31..H-65 через coworker-выжимку), closed-branches (8 веток), FRESH-HYPOTHESES-fable5 (FH-01..FH-17 целиком через coworker), FH-18/19 (упоминания), FH2-01..FH2-10 (все карточки), weakness-map, per-type-grid. Источники WebSearch 2024–2026: MRP/3-D-MRP, bit-plane slicing (CVPR'24 ArIB-BPS, BD-LVIC arXiv 2410.17814), Cool-chic/C3/FNLIC (CVPR'25), GLN/G9-V1 (AIT-2026 challenge arXiv 2606.17712), split-stream exe (US 7305541), PE `.reloc`/IAT, micro-diffusion Tweedie (arXiv 2603.08771), master-mixer. Карточки: `consilium/reports/FH3-01.md .. FH3-09.md`. Только чтение+WebSearch+артефакт; кода/БД/git нет.

- **FH3-01** — MRP (minimum-rate predictors): набор линейных предикторов с rate-оптимизированными ПЕРЕДАВАЕМЫМИ коэффициентами + контекст-группы; image (x-ray/mr, континуальные grayscale); якорь Matsuda MRP + 3-D-MRP (IEEE TMI 2017); −3…−8% self поверх MED16 (грубо); probe: референс-MRP на 8-bpp PGM-срезах (часы). Отличие от MED/GAP/CALIC — оптимизируется САМ предиктор под rate, не выбор из готовых.
- **FH3-02** — bit-plane MSB/LSB декомпозиция 16-бит остатка + кросс-плоскостной conditioning (структурный MSB, near-random LSB); image (x-ray/mr медицина 16-бит); якорь ArIB-BPS (CVPR'24), BD-LVIC (2410.17814, MSBV/LSBV); −2…−6% self (грубо, ЧЕСТНО может быть −1…−3% если LSB=чистый шум); probe: order-0 энтропия младших N бит + H(LSB|MSB) — МИНУТЫ. Отличие от H-50 (float через байт-backend, NO-GO) — integer-остаток после MED16.
- **FH3-03** — overfitted per-image нейрокодек (Cool-chic/C3/FNLIC): лёгкий ДЕТЕРМИНИЗИРУЕМЫЙ декодер, обучаемый на одном изображении, веса в потоке; image (ptt5/x-ray/mr); якорь FNLIC (CVPR'25, ≈JPEG-XL, декодер <3k MAC/пиксель); потолок −10…−30% self (грубо, realizable меньше после чарджа весов); probe: готовый FNLIC на ptt5/x-ray/mr (часы, GPU) — сначала как ПОТОЛОК-инструмент, rail только по операторскому сигналу.
- **FH3-04** — sparse Gated Linear Network (GLN) как вход CM-микшера: backprop-free локальное обучение, hashed n-gram контексты → constant-time integer-лукап, wire=0; text (усилить −12.1% margin) + code/db/exe; якорь G9-V1 (AIT-2026, arXiv 2606.17712) + GLN (AAAI'20); −1.5…−4% self text (грубо); probe: GLN-предиктор log-loss на dickens/webster vs одиночные компоненты Cubrim + корреляция ошибок (часы). Отличие от FH2-01 (RWKV, backprop-through-time, вшитые веса) — online-обучение из нуля.
- **FH3-05** — полное split-stream разложение инструкций (opcode/ModRM/SIB/disp/imm → типизированные потоки, per-stream CM); exe (mozilla dense-.text, ooffice), binary; якорь split-stream патенты US 7305541/6907516 + OpenZL; −3…−8% self поверх E8E9-BCJ (грубо); probe: готовый length-дизассемблер + разложить .text + сжать потоки текущим CLI (часы). Отличие от FH-07/09 (только call/jump-таргеты) — ВСЕ поля.
- **FH3-06** — relocation/IAT-aware: un-relocate `.reloc`-патченных абсолютных адресов (вычесть ImageBase→RVA) + дельта IAT; exe PE (ooffice/mozilla), ELF `.rela`; якорь PE reloc DELTA-механика + un-relocation-table патент US 11816471; −2…−6% self поверх E8E9 (грубо, ноль если нет `.reloc`, fail-closed); probe: распарсить `.reloc` + un-relocate + текущий CLI (часы). wire=0 (таблица уже в файле). Слепой угол BCJ — абсолютные адреса.
- **FH3-07** — Tweedie-denoised/micro-diffusion online-оценка вероятности как замена/дополнение StateMap в разреженных order-6..11 контекстах (наследование оценки по бинарному дереву); backend все CM-типы; якорь arXiv 2603.08771 (2026); −1…−3% self (грубо, лечит корень Gotcha #9 — learning cost, не потолок H(X|ctx)); probe: реальный online log-loss dual-counter vs Tweedie в редком хвосте (часы). Отличие от FH-01 (StateMap-автомат) — регрессионное сглаживание.
- **FH3-08** — иерархический structure+master микшер + адаптивный per-context learning-rate + match как отдельный слой; backend (text/code/db margin); якорь master-mixer (AIT-2026) + fx2-cmix + AdaLomo (ACL'24); −1…−3% self (грубо, улучшение АГРЕГАЦИИ, не новый сигнал); probe: оффлайн-микс на залогированных логит-выходах Cubrim (часы, свой кодек не трогать). Отличие от FH-05 (2-layer fixed-LR) — глубже топология + обучаемый LR.
- **FH3-09** — image-specific SSE/APM над 2D-соседским контекстом активности остатков MED16 (CALIC error-feedback bias-cancellation); image (x-ray/mr/ptt5, ДЁШЕВО); якорь CALIC bias-cancellation + SSE/APM (paq/zpaq); −1…−4% self поверх MED16 (грубо); probe: средний остаток по квантованным контекстам активности — МИНУТЫ (если несмещён → NO-GO). Самая дешёвая image-карточка по стоимости реализации (1–3 дня).

**Рекомендованная первая волна проб (по margin-gain × дешевизна пробы, приоритет image):** (1) **FH3-09 probe-1** — минуты (замер контекст-bias остатков MED16; дешевле всех, прямой инкремент к тончайшему image-margin); (2) **FH3-02 probe-1** — минуты (энтропия LSB + H(LSB|MSB) на x-ray/mr; решает, есть ли что брать bit-plane split'ом); (3) **FH3-01 probe-1** — часы (референс-MRP на PGM-срезах x-ray; самый большой ожидаемый image-gain −3…−8%); (4) **FH3-05 probe-1** — часы (split-stream .text mozilla/ooffice текущим CLI; решает судьбу дорогой exe-стройки, как FH2-03); (5) **FH3-06 probe-1** — часы (un-relocate `.reloc` ooffice; дёшево, ортогонально BCJ); (6) **FH3-04 probe** — часы (GLN log-loss на dickens; backend-margin text+перенос на все типы); (7) **FH3-08 probe** — часы (оффлайн-микс на логах, low-risk страховка); (8) **FH3-07 probe** — часы (Tweedie vs StateMap в редком хвосте). Нейрокарточка **FH3-03** — потолок-замер (часы, GPU) сначала как инструмент приоритизации; rail-путь только по операторскому сигналу о готовности к детерминированной нейрозависимости.

**Покрытие WAVE 3:** image — FH3-01/02/03/09 (4 карточки, высший приоритет — margin −4.6%); exe — FH3-05/06 (+ FH3-04 доборно); backend text — FH3-04/07/08; code/database — FH3-04/07/08 (backend-перенос); binary — FH3-05 (split-stream). Инварианты соблюдены во всех карточках: lossless byte-exact round-trip (cmp=0) + fail-closed decode + competitive-min (ноль регрессии по построению) + целочисленный детерминизм для реального кодека; все преобразования либо wire=0 (адаптивные: FH3-04/07/08/09), либо явно чарджированы отдельной ветвью декодера (Gotcha #6: FH3-01 коэффициенты, FH3-02 разрез, FH3-05 границы полей); ни одной передаваемой по-значной перестановки (Gotcha #7: FH3-06 использует `.reloc` уже в файле, FH3-05 — реассемблируемая по-полевая раскладка); все пробы меряют РЕАЛЬНЫЙ online log-loss/размер, не идеальную H(X|ctx) (Gotcha #9: явно в FH3-04/07). FH3-03 (нейро) честно помечена: encode 100–1000× медленнее — потолок-инструмент, rail условно при детерминизации.

WAVE 3 DONE: FH3-01..FH3-09, рекомендованная первая проба = FH3-09 (минуты, контекст-bias MED16 на x-ray/mr — дешевле всех и целит в тончайший image-margin −4.6%).

### WAVE 3 PROBE RESULTS (2026-07-23, Opus probe-orchestrator, реальные числа, worktree /home/dev/cubr-wave3-probes)

Корпус x-ray/mr/ptt5 скопирован с VERIFY-ноды (root@100.118.134.82:/root/corpus-full), размеры совпадают с манифестом. Пробы — реальные харнессы (numpy + реальный CLI), не фабрикация.

- **PROBE FH3-09: x-ray −1.9109% end-to-end (byte-exact, RT cmp=0) → GO (x-ray) / NO-GO (mr) / N/A (ptt5).**
  Причинная APM bias-cancellation (per-context running-mean истинной ошибки предсказания над MED16, декодер-воспроизводимая). order-0 остаток: x-ray −2.16% (потолок контекста −3.34%); mr +11.6% ХУЖЕ (структура mr — в дисперсии/форме, не в среднем — уже эксплуатируется BwtRans; ctx-потолок −20.9% = это FH-14-территория, не FH3-09). Реальный кодер: `cubrim compress --value-scheme bwt-rans` на остатке воспроизводит shipped MODE_MED16 nested ТОЧНО (xray_plain.rans=3771594==3771607−13). bias-corrected: x-ray 3771594→3699491 (−1.912%), mr 2098050→2182501 (+4.02%). **Реализовано в shipped-рейле** как второй MODE_MED16 competitive-min вариант (apm-флаг в старшем бите width, plain-контейнер байт-идентичен); commit bae50473. Реальный CLI: x-ray −1.9109% RT cmp=0 apm=1, mr байт-идентичен (apm=0, min() держит plain), ноль регрессии по построению. Image-тип 0.311949→0.308147. Ключ: x-ray остаток ≈order-0-несжимаем (BwtRans 7.12 vs H0 6.96 b/sample) ⇒ предикторный выигрыш проходит end-to-end; mr уже глубоко эксплуатирован. → guarded meta-13 DB-write подготовлен (монитор применяет), карточка → validated/WIN.
- **PROBE FH3-02: x-ray H0(low4)=4.000 (чистый шум), cross-plane gain ≤0.02 bit → NO-GO.**
  MSB/LSB bit-plane split 16-бит остатка. x-ray: младшие 1..5 бит = МАКСИМАЛЬНО случайны (H0=L точно), conditioning на MSB даёт ≤0.02 бит — LSB независим (точное условие NO-GO карточки). mr: младшие биты структурированы (H0(low1)=0.94), но cross-gain отражает СОВМЕСТНУЮ энтропию, которую BwtRans-бэкенд УЖЕ бьёт (реальный mr 3.37 < order-0 joint 4.13 b/sample); split = факторизация, не новая информация (Gotcha #11). Дешёвая структурная часть (без нейро-приора FH3-03) субсумирована бэкендом. → NO-GO, journal.
- **PROBE FH3-01: x-ray +1.85% (order-0 grouped-LS lower bound) / mr −25% ХУЖЕ → PARTIAL, перекрывается FH3-09.**
  Per-image оптимизированный 10-tap grouped-LS предиктор (нижний bound на MRP): x-ray бьёт MED16 на +1.17% (global) / +1.85% (M=4 группы); mr −25% ХУЖЕ (линейный предиктор проваливается на мульти-срезовом объёме, где медианный MED16 робастен). x-ray-headroom перекрывается предикторным выигрышем FH3-09 (та же информация); полный референс-MRP = тяжёлая 2-проходная стройка ради неопределённого инкремента над FH3-09. → DEFER полный MRP; FH3-09 берёт дешёвую реализуемую x-ray-часть.

**Сводка WAVE3-проб (image-ветка):** единственный реализуемый рычаг из трёх карточек — улучшение x-ray-предиктора ~2% (FH3-09 реализован, −1.91% byte-exact); mr глубоко эксплуатирован MED16+BwtRans и сопротивляется всем трём; ptt5 = bilevel CM2 (вне scope MED16-карточек — отдельный рычаг: 2D above-row модель, GeoCM-хэндофф D→A). exe-ветка (FH3-05/06): mozilla = tar ELF Alpha/Tru64 (не x86 PE) ⇒ x86 split-stream/reloc применимы только к ooffice (10% байт exe-типа) — сильно разбавлено; ветка B подтвердила NO-GO (Gotcha #11). Продуктивная граница backend — модель (FH3-04/07/08), не трансформы.

### GeoCM (D→A порт) — image-класс ДОМИНИРОВАН (2026-07-23, реализовано в shipped-рейле)

Ветка D (research) построила изолированный next-gen кодек GeoCM (`/home/dev/cubr2-geocm`, реинкарнация N-dim-cube в форме, переживающей информ-консервацию Gotcha #7: координаты — ДЕТЕКТИРОВАННЫЕ header-константы, не передаваемая карта). Хэндофф D→A: порт в shipped image-рейл. Заморожённый референс `A-port-ref-image = b799a3c` (v6).

- **GeoCM (MODE_GEOCM=17, competitive-min):** детект нативной оси (stride autocorrelation) → 2D above-row + o1/o2/MED контексты → 8-модельный integer logistic mixer + 2 APM → carryless range coder. Гейт: best-stride byte-equality ≥0.15·2²⁰ & размер ≤12МБ (только image; kennedy.xls/sum гейт-fire но GeoCM проигрывает; текст/exe reject). Fail-closed (внутренний CG2 FNV-64). Commit b293daac.
- **Реальные числа (shipped CLI, RT cmp=0, mode=17 выигрывает):** ptt5 44262→29918 (**−32.41%**, per-file rank 5→1, ниже xz 0.0777), x-ray 3771607→3663004 (**−2.88%**, бьёт FH3-09 3699535), mr 2098063→2077533 (**−0.98%**). Воспроизводит frozen v6 числа точно (+6 обёртка). Image-тип 0.311949→**0.304381** (−2.43% self), overall 0.191285→0.190829. Ноль per-file регрессии (аддитивный competitive-min). Тесты geocm_roundtrips_and_fails_closed (RT + fail-closed) PASS.
- **Урок:** детектированная-осевая geo-контекст-модель — решающий image-рычаг, которого не было у 1D-рейла: ptt5 (bilevel fax, сильная вертикальная ось) −32% и в rank 1; smooth 16-бит медицина (x-ray/mr) через above-row+MED входы микшера. Одновременно ядро Cubrim-2 и сильнейший рычаг Cubrim-1 image-margin (тончайший −4.6% → расширен). GEOCM-IMG → validated/WIN. Guarded meta-SQL подготовлен (динамическая база поверх FH3-09; монитор применяет).


### WAVE 3 — FH3-05 exe split-stream — NO-GO (2026-07-23, ВЕТКА-B)
ooffice PE32 i386 dense .text 5 281 585 B, capstone-5 field-split (byte-exact partition, 0.01% escapes). Σ split-streams 2 472 883 ≥ монолит Cubrim 2 416 003 (+2.35%); xz cross-check +3.60%. NO-GO до charge границ — монолитный энтропийный кодер уже ловит вертикальную field-корреляцию, split рвёт кросс-field conditioning (Gotcha #11). Journal-only. Harness probes-b/fh3_05_split.py.


### WAVE 3 — FH3-06 exe un-relocate — NO-GO (2026-07-23, ВЕТКА-B)
ooffice PE32, ImageBase 0x1e000000, 105k HIGHLOW reloc, coverage 6.83%, transform RT verified (wire=0). Un-relocate Cubrim 2 682 682 vs orig 2 683 924 = −1 242 B (−0.046%); xz −0.03%. Оценка −2…−6% не реализовалась: адресная энтропия уже поймана LZ/CM (absolute VA high-byte near-constant 0x1e). NO-GO (ниже worth-building порога). Journal-only. Harness probes-b/fh3_06_unreloc.py.


### WAVE 3 — FH3-04 text GLN-предиктор — NO-GO (2026-07-23, ВЕТКА-B)
dickens 1M, честный online CM-микс (wire=0). GLN как доп.вход: o0..8 база −0.43%, +match+high-orders −0.39%, +APM/SSE −0.08%. Одна SSE-ступень субсумировала ~80% сигнала GLN (мультипликативный gating исправлял miscalibration микса = функция APM). Реальный Cubrim (цепочка APM+StateMap) поглотит остаток → ~0. Gotcha #11. NO-GO, journal-only. Harness probes-b/fh3_04_gln_v4.py.


### WAVE 3 — FH3-08 structure+master mixer / adaptive-LR — NO-GO (2026-07-23, ВЕТКА-B)
dickens 1M, честный online микс + APM (2.196 bpc). Изолированный adaptive-LR (b1) = −0.01% < 0.5%. cm2.rs УЖЕ имеет 2-layer + 5 context-mixers (match-state слой) → layer-2 → 2-stage APM; richer-топология (6th mixer/richer l2/3rd APM) уже пробована и REGRESSED. Топология микшера исчерпана. NO-GO, journal-only. Harness probes-b/fh3_08_mixer.py.


### WAVE 3 — FH3-07 Tweedie sparse-context denoiser — NO-GO (2026-07-23, ВЕТКА-B)
dickens, честный online, Tweedie-tree как доп.вход. Слабый fast-counter база @1M −2.19% (ложно-плюс), но faithful lpaq StateMap база (порт cm2.rs, 1.9343 bpc) @1M −0.30% < 0.5% → NO-GO. StateMap субсумирует sparse-context вклад Tweedie; совпадает с dead-ends cm2.rs (StateMap replacement / 2nd indirect regressed). Gotcha #9: fidelity прокси решает. Journal-only. Harness probes-b/fh3_07_statemap.py.


### WAVE 3 — ВЕТКА-B (exe + text/database) ИСЧЕРПАНА — 5/5 NO-GO (2026-07-23)
FH3-05 split-stream (+2.35%), FH3-06 un-relocate (−0.046%), FH3-04 GLN (−0.08% post-APM), FH3-08 structure/master mixer (−0.01%), FH3-07 Tweedie sparse-ctx (−0.30% faithful StateMap). Сквозной итог: backend Cubrim CM2 (dual counter + lpaq StateMap + 2-layer context-mixer + 2-stage APM + match/word/indirect) уже субсумирует инкрементальные WAVE-3 backend-идеи; exe-структурные трансформы поглощены competitive-min CM/LZ. Margin ветки B не увеличен. Методологический якорь: FH3-07 fidelity прокси (fast-counter −2.19% ложно-плюс → lpaq StateMap −0.30% NO-GO), Gotcha #9. Все harnesses в cubr-branch-b-work/probes-b/.
