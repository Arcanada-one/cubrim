...
- **Context:** H-18b showed 74.84% RAW cross-file redundancy on the dedup-corpus, but raw redundancy is not a win — the honest test is whether a shipped shared-dictionary scheme beats what existing tools already extract. Size model `docs/ephemeral/research/probe_h18_sizemodel.py` compares corpus-total compressed bytes on the SAME redundant corpus.
