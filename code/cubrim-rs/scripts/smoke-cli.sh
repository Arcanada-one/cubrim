...
grep -Eq 'compress [0-9.]+ (ms|s)' "${TMPDIR}/compress.err" || die "compress stderr missing compress time"
grep -q 'MB/s' "${TMPDIR}/compress.err" || die "compress stderr missing throughput"
grep -q 'decompressed:' "${TMPDIR}/decompress.err" || die "decompress stderr missing decompressed line"
grep -Eq 'decompress [0-9.]+ (ms|s)' "${TMPDIR}/decompress.err" || die "decompress stderr missing decompress time"
