...
fn bench_cubr0027_aggregate() {
    ...
    println!("{:<18} {:>8} {:>8} {:>8}  {}", "file", "T4_bytes", "T5_bytes", "delta", "mode");
    ...
    for f in FILES {
        ...
        println!("{:<18} {:>8} {:>8} {:>8}  {}",
            f.name, t4_blob.len(), t5_blob.len(),
    ...
    println!("{:<18} {:>8} {:>8} {:>8}", "TOTAL", t4_total, t5_128_total,
    ...
    println!("{:>12} {:>12} {:>10} {:>12}", "min_ctx", "total_bytes", "aggregate", "delta_vs_t4");
    ...
    for &min_ctx in min_ctx_values {
        ...
        println!("{:>12} {:>12} {:>10.6} {:>+12.6}", min_ctx, total, agg, delta);
        if agg < best_a_ratio {
            best_a_ratio = agg;
            best_a_ctx = min_ctx;
            best_a_bytes = total;
    ...
    println!("");
    println!("Option A best: min_ctx={}, aggregate={:.6}, bytes={}",
        best_a_ctx, best_a_ratio, best_a_bytes);
    println!("Option A best delta vs T4: {:.6}",
    ...
    if best_a_ratio < T4_BASELINE_AGG {
        println!("Option A: GO — beats T4 by {:.6}", T4_BASELINE_AGG - best_a_ratio);
    } else {
        println!("Option A: WORSE than T4 by {:.6} — no improvement on this corpus",
            best_a_ratio - T4_BASELINE_AGG);
    }
    println!("Target from spike: 0.547730 (Python twin, model-only, order-1 unserialized)");
    println!("Real Option A best: {:.6} at min_ctx={}", best_a_ratio, best_a_ctx);
