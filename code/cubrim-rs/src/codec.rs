...
// ── Option B helper (bench only): 2-level wire (order-2 + order-0, no order-1) ──
//
// The Python twin's size model used order-1 in the bitstream but did NOT serialize
// order-1 tables. For a real codec the choices are:
//   Option A (3-level): write order-1 tables → higher header overhead, lower bitstream
//   Option B (2-level): skip order-1 tables → lower header overhead, higher bitstream
//                       (order-1 positions use order-0 for coding)
//
// This function implements Option B for bench comparison only. It is NOT the primary
// value-scheme codec — the primary uses Option A (order2_context_huffman_encode above).

/// Build Option-B context tables: only order-0 (fallback) and order-2 qualifying tables.
/// Order-1 intermediate tables are intentionally omitted.
pub(crate) fn order2_build_context_tables_2level(
    seq_codes: &[usize],
    n_distinct: usize,
    min_ctx_count: u16,
) -> Vec<CtxEntry> {
    ...
    if seq_codes.is_empty() || n_distinct == 0 {
        return vec![CtxEntry::Order0 { code_len: vec![0u8; n_distinct] }];
    }
    ...
    // Order-2 tables only (no order-1)
    ...
    let mut result = Vec::with_capacity(1 + order2_entries.len());
    result.push(CtxEntry::Order0 { code_len: fallback_code_len });
    result.extend(order2_entries);
    result
}

/// Encode using Option B (2-level wire): order-2 + order-0 only, no order-1 tables.
pub(crate) fn order2_context_huffman_encode_2level(
    seq_codes: &[usize],
    n_distinct: usize,
    min_ctx_count: u16,
) -> Vec<u8> {
    if seq_codes.is_empty() {
        ...
        out.push(0u8);
        out.extend_from_slice(&vec![0u8; n_distinct]);
        return out;
    }

    let entries = order2_build_context_tables_2level(seq_codes, n_distinct, min_ctx_count);

    let canonical_codes: Vec<Vec<(u32, u8)>> = entries.iter()
        .map(|e| crate::huffman::assign_canonical_codes(e.code_len()))
        .collect();

    // Build lookup maps (2-level: only order-0 and order-2).
    use std::collections::HashMap;
    let mut order0_idx = 0usize;
    ...
    for (i, entry) in entries.iter().enumerate() {
        match entry {
            ...
            CtxEntry::Order1 { .. } => {} // not generated by 2-level builder
        }
    }
    ...
    for (i, &code) in seq_codes.iter().enumerate() {
        let (p2, p1) = order2_ctx_at(seq_codes, i);
        // 2-level: order-2 → order-0 (no order-1)
        let table_idx = order2_map.get(&(p2, p1)).copied().unwrap_or(order0_idx);
        let (codeword, length) = canonical_codes[table_idx][code];
        bit_acc = (bit_acc << length) | (codeword as u64);
        bit_count += length as u32;
        while bit_count >= 8 {
...
/// Size estimate for Option B (2-level wire).
pub(crate) fn order2_context_huffman_size_2level(
    seq_codes: &[usize],
    n_distinct: usize,
    min_ctx_count: u16,
) -> usize {
