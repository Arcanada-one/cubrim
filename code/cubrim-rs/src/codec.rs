...
///
/// Note on containers: the mode dispatch below hands nested blobs to the
/// per-mode decoders, which re-enter through `decode()` and therefore use the
/// default limits rather than inheriting `limits`. Threading limits through
/// every container decoder is a wider change than this merge should make; the
/// depth guard still bounds recursion, and the per-block gate still applies to
/// each nested single-block payload.
