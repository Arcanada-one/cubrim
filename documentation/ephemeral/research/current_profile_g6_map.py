#!/usr/bin/env python3
"""Frozen-artifact, full-binary mapper for the NEW-24 G6 campaign."""
...
class MappingError(ValueError):
    """A frozen input cannot satisfy the G6 contract."""
...
def split_instruction_map(
    ...
    while start < len(canonical_rows):
        ...
        start = end
    manifest = {"schema": "cubr-new24-g6-map-parts-v1", "part_count": len(parts), "row_count": len(canonical_rows), "full_uncompressed_bytes": len(full), "full_uncompressed_sha256": hashlib.sha256(full).hexdigest(), "parts": evidence}
...
def summarize_file(cell: str, record_a: Mapping[str, Any], record_b: Mapping[str, Any]) -> dict[str, Any]:
    require(record_a.get("schema") == record_b.get("schema") == "cubr-new24-g6-record-v1", "record schema mismatch")
    ...
    return {"schema": "cubr-new24-g6-file-family-summary-v1", "cell": clean_field(cell, "cell"), "families": source, "material_families": material_source, "source_families": source, "material_source_families": material_source, "emitted_families": emitted, "material_emitted_families": material_emitted, "cross_file_reduction_performed": False, "material_threshold_percent_either_record": 5.0, "repeatability_delta_max_percentage_points": 1.0}
...
def build_g6_admission_seal(*, binary_build_id: str, binary_sha256: str,
...
def _valid_g6_reuse_identity(identity: Any) -> bool:
...
def g6_reuse_decision(existing: Mapping[str, Any], candidate: Mapping[str, Any]) -> str:
    if not (_valid_g6_reuse_identity(existing) and _valid_g6_reuse_identity(candidate)):
...
def _g6_map_artifacts(args: argparse.Namespace) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    manifest_path = _input(args.input_root, args.map_manifest)
    manifest = _json_input(manifest_path)
    require(manifest.get("schema") == "cubr-new24-g6-map-parts-v1",
    ...
    require(isinstance(summary, dict) and
            summary.get("schema") == "cubr-new24-g6-static-map-summary-v3",
...
def run_command(args: argparse.Namespace) -> None:
    ...
    if args.command == "normalize-elf":
        ...
        write_new_bytes(_output(args.output_root, args.summary_out), json_bytes({
            "schema": "cubr-new24-g6-normalized-elf-v1",
        ...
        map_artifacts, toolchain = _g6_map_artifacts(args)
        seal = build_g6_admission_seal(
        ...
        manifest_path = _input(args.input_root, args.map_manifest)
        manifest = _json_input(manifest_path)
        require(manifest.get("schema") == "cubr-new24-g6-map-parts-v1", "map manifest schema mismatch")
        manifest_parent = PurePosixPath(str(args.map_manifest)).parent
        declared_parts = manifest.get("parts", [])
