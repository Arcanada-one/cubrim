...
N = 5345280                    # xml, canonical Silesia
LANDED_WALL = 58.000           # G2 attribution, plain pinned profiling wall
LANDED_BOUND = 10.707          # G2 combined outer bound, xml/max (clean cell)
...
def main():
    ...
    print(f"\n=== xml ({N:,} B = {mib:.4f} MiB) — same host, pin 0-15, "
    ...
    print(f"  P1 within 2x of implied [{lo:.4f},{hi:.4f}]: {cub:.4f} -> "
          f"{'HOLDS' if lo <= cub <= hi else 'REFUTED'}")
    ...
    print(f"  P4 best case within 2x of dickens's {DICKENS_BEST} [{p4lo:.3f},{p4hi:.3f}]: {best:.3f} -> "
          f"{'HOLDS' if p4lo <= best <= p4hi else 'REFUTED'}")
