...
runner_literals=(
    ...
    'parse_perf_event_probe "$d/perf-stat-$safe.txt" "$event"'
    'perf-events.tsv'
    'status\":\"unsupported'
    'cmp --'
    '[[ $(sha "$src") == "$want_sha" ]]'
...
reject_runner_fixed 'found=$(/usr/bin/ps'
...
if [[ $SELF_MUTATION_TESTS == 1 ]]; then
    ...
    expect_runner_mutant_red process_scan_snapshot 's#/usr/bin/ps -eo pid=,ppid=,comm=,args= >"\$snapshot"#/usr/bin/ps -eo pid=,ppid=,comm=,args=#' 'runner missing literal: /usr/bin/ps -eo pid=,ppid=,comm=,args= >"$snapshot"'
    expect_runner_mutant_red perf_probe_event_arg 's/parse_perf_event_probe "\$d\/perf-stat-\$safe.txt" "\$event"/parse_perf_event_probe "$d\/perf-stat-$safe.txt"/' 'runner missing literal: parse_perf_event_probe "$d/perf-stat-$safe.txt" "$event"'
