#!/usr/bin/env python3
"""TDD and security controls for the NEW-24 G6 full-binary mapper."""
...
import current_profile_g6_map as g4
...
class GzipTests(unittest.TestCase):
    def test_required_g6_map_schemas_are_exact(self) -> None:
        ...
        self.assertTrue({
            "cubr-new24-g6-normalized-elf-v1",
            "cubr-new24-g6-static-map-summary-v3",
            "cubr-new24-g6-map-parts-v1",
            "cubr-new24-g6-map-admission-seal-v1",
        }.issubset(emitted_schemas), emitted_schemas)

    def test_every_emitted_schema_is_g6_namespaced_without_fresh_output_residue(self) -> None:
        source_tree = ast.parse(Path(g4.__file__).read_text(encoding="utf-8"))
        emitted_schemas = [
            value.value
            for node in ast.walk(source_tree)
        ...
        self.assertTrue(
            all(schema.startswith("cubr-new24-g6-") for schema in emitted_schemas),
            emitted_schemas,
        )

        _, manifest = g4.split_instruction_map(build_rows(), max_rows=1, part_prefix="g6-map")
...
class AdmissionSealTests(unittest.TestCase):
    def seal(self, **overrides: object) -> dict:
        ...
        return g4.build_g6_admission_seal(**arguments)
    ...
    def test_complete_identical_identity_is_reusable(self) -> None:
        ...
        self.assertEqual(
            g4.g6_reuse_decision(identity, copy.deepcopy(identity)),
        ...
        for axis, variants in axis_field_variants.items():
            for fields in variants:
                with self.subTest(identity_axis=axis, missing_fields=fields):
                    ...
                    self.assertEqual(
                        g4.g6_reuse_decision(incomplete, copy.deepcopy(incomplete)),
                        "REJECTED_IDENTITY_MISMATCH",
                    )
                    self.assertEqual(
                        g4.g6_reuse_decision(identity, incomplete),
                        "REJECTED_IDENTITY_MISMATCH",
                    )
                    self.assertEqual(
                        g4.g6_reuse_decision(incomplete, identity),
                        "REJECTED_IDENTITY_MISMATCH",
                    )
    ...
    def test_partial_identity_is_rejected(self) -> None:
        ...
        for partial in partials:
            with self.subTest(fields=sorted(partial)):
                self.assertEqual(
                    g4.g6_reuse_decision(partial, copy.deepcopy(partial)),
                    "REJECTED_IDENTITY_MISMATCH",
                )
    ...
    def test_invalid_identity_shapes_and_types_are_rejected(self) -> None:
        ...
        for label, malformed in invalid_identities.items():
            with self.subTest(invalid_identity=label):
                self.assertEqual(
                    g4.g6_reuse_decision(malformed, copy.deepcopy(malformed)),  # type: ignore[arg-type]
                    "REJECTED_IDENTITY_MISMATCH",
                )
                self.assertEqual(
                    g4.g6_reuse_decision(identity, malformed),  # type: ignore[arg-type]
                    "REJECTED_IDENTITY_MISMATCH",
                )
    ...
    def test_g4_identity_reuse_is_rejected(self) -> None:
        ...
        for axis, variants in mutations.items():
            for changes in variants:
                with self.subTest(identity_axis=axis, changes=changes):
                    ...
                    candidate.update(changes)
                    self.assertEqual(
                        g4.g6_reuse_decision(g4_identity, candidate),
                        "REJECTED_IDENTITY_MISMATCH",
                    )
        ...
        self.assertEqual(fresh["schema"], "cubr-new24-g6-map-admission-seal-v1")
    ...
    def test_seal_admission_uses_single_output_join(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            part_blobs, manifest = g4.split_instruction_map(
                build_rows(), max_rows=10, part_prefix="g6-map"
            )
            ...
            with mock.patch.object(
                g4, "build_g6_admission_seal", wraps=g4.build_g6_admission_seal
            ) as constructor:
            ...
            self.assertEqual(seal["schema"], "cubr-new24-g6-map-admission-seal-v1")
    ...
    def test_build_and_reduce_cli_round_trip_multipart_map(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            g4.run_command(parser.parse_args([
                ...
                "--mapping-schema-sha256", "a" * 64, "--map-part-prefix", "g6-map",
