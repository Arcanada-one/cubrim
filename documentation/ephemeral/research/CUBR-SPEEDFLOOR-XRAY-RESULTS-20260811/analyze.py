...
N = 8474240                    # x-ray, canonical Silesia
LANDED_WALL = 6.200            # G2 attribution, plain pinned profiling wall (x-ray/max)
LANDED_BOUND = 55.556          # as published (shell INCLUDED)
BOUND_CONSISTENT = 51.020      # shell EXCLUDED, the basis the CM2 cells use
...
def main():
    ...
    print(f"\n=== x-ray/max ({N:,} B = {mib:.4f} MiB) — same host, pin 0-15, "
    ...
    for t in sorted(med, key=lambda x: mib / med[x], reverse=True):
        tp = mib / med[t]
        st = {"cubrim": "max", "xz": "-9", "zstd": "-19", "brotli": "-q11",
    ...
    print(f"  P2 same-round ratio vs xz -9 < 100x: {medr['xz']:.0f}x -> "
          f"{'HOLDS' if medr['xz'] < 100 else 'REFUTED'}")
    bc = cub * BOUND_CONSISTENT
    print(f"  P3 best case >= {FIELD_9TH[1]} on BOTH bases: published {best:.2f}, consistent {bc:.2f} -> "
          f"{'HOLDS' if (best >= FIELD_9TH[1] and bc >= FIELD_9TH[1]) else 'REFUTED'}")
    print(f"  P4 consistent-basis best case >= 52.71 (bzip2, 8th): {bc:.2f} -> "
          f"{'HOLDS' if bc >= 52.71 else 'REFUTED'}")
    print(f"\n  both bases: published 55.556x -> {best:.2f} MiB/s ; consistent 51.020x -> {bc:.2f} MiB/s")
