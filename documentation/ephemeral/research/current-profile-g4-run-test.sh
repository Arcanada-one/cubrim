#!/usr/bin/env bash
# Static, runtime, and mutation-sensitive contract tests for the NEW-24 G4 runner.
...
readonly TEST_DIR
readonly SELF=$TEST_DIR/current-profile-g4-run-test.sh
RUNNER=${RUNNER:-$TEST_DIR/current-profile-g4-run.sh}
MAPPER=${MAPPER:-$TEST_DIR/current_profile_g4_map.py}
SELF_MUTATION_TESTS=${SELF_MUTATION_TESTS:-1}

fail() {
    printf 'current_profile_g4_contract=FAIL reason=%s\n' "$1" >&2
    exit 1
}

invalid() {
    printf 'current_profile_g4_contract=HARNESS_INVALID reason=%s\n' "$1" >&2
    exit 2
}
...
runner_literals=(
    '/root/cubr-new24-full-binary-g4-src'
    '/root/cubr-new24-full-binary-g4-target'
    '/root/cubr-new24-full-binary-g4-20260809'
    ...
    'current_profile_g4_fake_cargo=PASS'
...
self_test_rc=$?
set -e
if ! { (( self_test_rc == 0 )) && [[ $self_test_output == 'current_profile_g4_self_test=PASS' ]]; }; then
...
fake_cargo_rc=$?
set -e
if ! { (( fake_cargo_rc == 0 )) && [[ $fake_cargo_output == 'current_profile_g4_fake_cargo=PASS' ]]; }; then
...
[[ $publish_output == 'current_profile_g4_publish_test=PASS' ]] ||
...
[[ $cgroup_output == 'current_profile_g4_cgroup_test=PASS' ]] ||
...
[[ $cgroup_precommit_output == 'current_profile_g4_cgroup_precommit_test=PASS' ]] ||
...
[[ $publish_write_output == 'current_profile_g4_publish_write_test=PASS' ]] ||
...
[[ $publish_tamper_output == 'current_profile_g4_publish_tamper_test=PASS' ]] ||
...
[[ $hard_deadline_output == 'current_profile_g4_hard_deadline_test=PASS' ]] ||
...
if [[ $SELF_MUTATION_TESTS == 1 ]]; then
    ...
    [[ $cgroup_live_output == 'current_profile_g4_cgroup_live_test=PASS' ||
       $cgroup_live_output == 'current_profile_g4_cgroup_live_test=SKIP reason=user-systemd-unavailable' ]] ||
    ...
    [[ $timeout_tree_output == *'current_profile_g4_timeout_tree_test=PASS' ]] ||
    ...
    [[ $publish_crash_output == 'current_profile_g4_publish_crash_test=PASS' ]] ||
...
if [[ $SELF_MUTATION_TESTS == 1 ]]; then
    ...
    if ! { (( CHILD_RC == 0 )) && [[ $CHILD_OUTPUT == 'current_profile_g4_contract=PASS' ]]; }; then
    ...
    if ! { (( CHILD_RC == 2 )) && [[ $CHILD_OUTPUT == current_profile_g4_contract=HARNESS_INVALID\ reason=runner\ not\ found\ or\ unsafe:* ]]; }; then
    ...
    expect_runner_mutant_red() {
        ...
        ! /usr/bin/grep -qF 'current_profile_g4_contract=PASS' <<<"$CHILD_OUTPUT" ||
            invalid "mutation emitted PASS: $label"
        /usr/bin/grep -qF "current_profile_g4_contract=FAIL reason=$expected_reason" <<<"$CHILD_OUTPUT" ||
    ...
    expect_self_test_mutant_red() {
        ...
        [[ $CHILD_OUTPUT == "current_profile_g4_self_test=FAIL reason=$expected_reason" ]] ||
    ...
    expect_runner_mutant_red output_path \
        's/cubr-new24-full-binary-g4-20260809/cubr-new24-full-binary-g4-later/' \
        'runner missing literal: /root/cubr-new24-full-binary-g4-20260809'
    ...
    expect_runtime_mutant_red disconnected_publisher_ancestry \
        ...
        'runner cgroup precommit counterexample failed: current_profile_g4_cgroup_precommit_test=FAIL disconnected-publisher-accepted'
...
printf 'current_profile_g4_contract=PASS\n'
