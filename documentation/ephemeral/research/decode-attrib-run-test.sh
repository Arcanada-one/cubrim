...
if [[ $SELF_MUTATION_TESTS == 1 ]]; then
    ...
    expect_mutant_red suite 's/"\$CARGO" test --release --test scheme_roundtrip/"\$CARGO" test --release scheme_roundtrip/' 'missing literal: "$CARGO" test --release --test scheme_roundtrip -- --nocapture)'
