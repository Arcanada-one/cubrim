...
[[ -f $OUT/results.tsv ]] || printf 'cell\tphase\tsample\tbuild\twall_s\trss_kib\n' > "$OUT/results.tsv"
[[ -f $OUT/roundtrips.tsv ]] || printf 'cell\tphase\tsample\tbuild\tstatus\n' > "$OUT/roundtrips.tsv"
...
for line in "${CELLS[@]}"; do
  ...
  for b in base current zero; do
    ...
    if [[ -f $archive && $(sha "$archive") == "$sum" ]]; then echo "compress $cell/$b: cached"; else
      scoped taskset -c "$PIN" timeout 3600 "$binary" compress --preset "$p" --quiet "$input" "$archive" || die "compress $cell/$b"
    fi
