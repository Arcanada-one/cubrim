#!/usr/bin/env bash
# Frozen NEW-24 G4 full-binary attribution runner. No selection or DB mutation.
set -Eeuo pipefail
IFS=$'\n\t'
export LC_ALL=C
...
readonly CODE_DIR=/root/cubr-new24-full-binary-g4-src
readonly PROFILE_TARGET=/root/cubr-new24-full-binary-g4-target
...
readonly OUT=/root/cubr-new24-full-binary-g4-20260809
...
readonly MAPPER_SOURCE=/root/cubr-new24-full-binary-g4-map.py
readonly MAPPER_TEST_SOURCE=/root/cubr-new24-full-binary-g4-map-test.py
readonly RUNNER_TEST_SOURCE=/root/cubr-new24-full-binary-g4-run-test.sh
readonly INSTRUMENT_REPO=${CUBR_INSTRUMENT_REPO:-/root/cubr-new24-full-binary-g4-instrument}
...
die() {
    ...
    printf 'current_profile_g4=VOID reason=%s\n' "$*" >&2
    exit 1
}
...
verify_instrument_provenance() {
    ...
    runner_path=documentation/ephemeral/research/current-profile-g4-run.sh
    mapper_path=documentation/ephemeral/research/current_profile_g4_map.py
    test_path=documentation/ephemeral/research/current-profile-g4-run-test.sh
    mapper_test_path=documentation/ephemeral/research/test_current_profile_g4_map.py
    ...
    /usr/bin/awk -v runner="$$" -v parent="$PPID" '
        $1 != runner && $1 != parent {
            ...
            if (comm ~ /^(cargo|rustc|rustup|perf)$/ ||
                args ~ /(cubrim|current-profile-g[34]-run|current_profile_g4_map[.]py)/) print
        }
    ' "$snapshot" >"$output"
}
...
admission() {
    ...
    /usr/bin/install -m 0444 -- "$MAPPER_SOURCE" "$dir/mapper-test-runtime/current_profile_g4_map.py"
    /usr/bin/install -m 0444 -- "$MAPPER_TEST_SOURCE" "$dir/mapper-test-runtime/test_current_profile_g4_map.py"
    require_deadline admission-mapper-tests
    (cd "$dir/mapper-test-runtime" && run_bounded 300 /usr/bin/python3 test_current_profile_g4_map.py) >"$dir/mapper-unit-test.txt"
...
seal = {
    "schema": "cubr-new24-g4-map-admission-seal-v1", "attempt": 9,
...
actual_ok = (
    ...
    summary.get("schema") == "cubr-new24-g4-static-map-summary-v3" and
...
sanitized = {
    "schema": "cubr-new24-g4-address-smoke-v1",
...
def publication_marker_bytes(final_path):
    return (f"schema=current-profile-g4-publication-v1\nstatus={status}\n"
...
self_test_fail() {
    printf 'current_profile_g4_self_test=FAIL reason=%s\n' "$1"
    exit 1
}
...
self_test() {
    ...
    /usr/bin/rm -rf -- "$audit_dir"
    printf 'current_profile_g4_self_test=PASS\n'
}
...
self_test_fake_cargo() {
    ...
    if [[ $(/usr/bin/wc -l <"$matches") != 1 ]] || ! /usr/bin/grep -qF 'cargo build --release' "$matches"; then
        /usr/bin/rm -rf -- "$dir"
        printf 'current_profile_g4_fake_cargo=FAIL\n'
        exit 1
    fi
    /usr/bin/rm -rf -- "$dir"
    printf 'current_profile_g4_fake_cargo=PASS\n'
}
...
self_test_cgroup() {
    ...
    if (( rc != 125 )) ||
       ...
       [[ $FAILURE_REASON != *'exact systemd ControlGroup'* ]]; then
        /usr/bin/rm -rf -- "$root"
        printf 'current_profile_g4_cgroup_test=FAIL\n'
        exit 1
    fi
    /usr/bin/rm -rf -- "$root"
    printf 'current_profile_g4_cgroup_test=PASS\n'
}
...
self_test_cgroup_live() {
    ...
    if ! /usr/bin/systemctl --user show-environment >/dev/null 2>&1; then
        printf 'current_profile_g4_cgroup_live_test=SKIP reason=user-systemd-unavailable\n'
        return 0
    fi
    root=$(/usr/bin/mktemp -d)
    result=$root/cgroup-live.tsv
    unit=current-profile-g4-cgroup-selftest-$$.service
    ...
    if (( rc == 0 )) || [[ ! -f $result ]] ||
       ...
       /usr/bin/systemctl --user is-active --quiet "$unit"; then
        printf 'current_profile_g4_cgroup_live_test=FAIL rc=%s output=%s\n' "$rc" "$output"
        /usr/bin/rm -rf -- "$root"
        exit 1
    fi
    /usr/bin/rm -rf -- "$root"
    printf 'current_profile_g4_cgroup_live_test=PASS\n'
}
...
self_test_cgroup_precommit() {
    ...
    if (( rc != 0 )) || [[ ! -f $final/TIMING-DONE.STAMP ]] || [[ -e $sentinel ]]; then
        ...
        /usr/bin/rm -rf -- "$root"
        printf 'current_profile_g4_cgroup_precommit_test=FAIL connected-publisher-rejected\n'
        exit 1
    fi
    /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
    /usr/bin/rm -rf -- "$root"
    ...
    if (( rc != 125 )) || [[ -e $final ]] || [[ ! -f $sentinel ]] ||
       [[ $(<"$sentinel") != "$expected_stop" ]]; then
        /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
        /usr/bin/rm -rf -- "$root"
        printf 'current_profile_g4_cgroup_precommit_test=FAIL disconnected-publisher-accepted\n'
        exit 1
    fi
    /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
    /usr/bin/rm -rf -- "$root"
    printf 'current_profile_g4_cgroup_precommit_test=PASS\n'
}
...
self_test_publish() {
    ...
    [[ ! -e $partial && ! -e $publishing && -f $final/TIMING-DONE.STAMP && -f $final/evidence-sha256.tsv &&
       ! -e $final/.TIMING-DONE.STAMP.pending ]] || {
        /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
        /usr/bin/rm -rf "$root"
        printf 'current_profile_g4_publish_test=FAIL\n'
        exit 1
    }
    ...
    /usr/bin/rm -rf "$root"
    printf 'current_profile_g4_publish_test=PASS\n'
}
...
self_test_publish_writes() {
    ...
    for mode in eintr short; do
        ...
        CUBR_PUBLISH_WRITE_TEST=$mode publish_campaign "$partial" "$publishing" "$final" "$late" \
            ...
            printf 'current_profile_g4_publish_write_test=FAIL mode=%s\n' "$mode"; exit 1;
        }
    ...
    if (( rc == 0 )) || [[ -e $final ]]; then
        printf 'current_profile_g4_publish_write_test=FAIL mode=zero\n'; exit 1
    fi
    /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
    /usr/bin/rm -rf "$root"
    printf 'current_profile_g4_publish_write_test=PASS\n'
}
...
self_test_publish_tamper() {
    ...
    for mode in tamper-marker tamper-content; do
        ...
        set -e
        if (( rc == 0 )) || [[ -e $final ]]; then
            printf 'current_profile_g4_publish_tamper_test=FAIL mode=%s\n' "$mode"; exit 1
        fi
        /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
        /usr/bin/rm -rf "$root"
    done
    printf 'current_profile_g4_publish_tamper_test=PASS\n'
}
...
self_test_timeout_tree() {
    ...
    if (( rc == 0 )) || /usr/bin/kill -0 "$parent_pid" 2>/dev/null || /usr/bin/kill -0 "$child_pid" 2>/dev/null; then
        /usr/bin/rm -rf "$root"
        printf 'current_profile_g4_timeout_tree_test=FAIL\n'
        exit 1
    fi
    /usr/bin/rm -rf "$root"
    printf 'current_profile_g4_timeout_tree_test=PASS\n'
}
...
self_test_publish_crashes() {
    ...
    for point in pending-written tree-fsynced publishing-renamed marker-renamed; do
        ...
        rc=$?
        set -e
        (( rc != 0 )) || { printf 'current_profile_g4_publish_crash_test=FAIL point=%s\n' "$point"; exit 1; }
        [[ ! -e $partial/TIMING-DONE.STAMP ]] || {
            printf 'current_profile_g4_publish_crash_test=FAIL point=%s partial-marker\n' "$point"; exit 1;
        }
        accepted=0
        ...
        (( accepted == 0 )) || {
            printf 'current_profile_g4_publish_crash_test=FAIL point=%s accepted\n' "$point"; exit 1;
        }
        /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
        /usr/bin/rm -rf "$root"
    done
    printf 'current_profile_g4_publish_crash_test=PASS\n'
}
...
self_test_hard_deadline() {
    ...
    (( rc != 0 )) || { printf 'current_profile_g4_hard_deadline_test=FAIL accepted\n'; exit 1; }
    [[ ! -e $final && ! -e $late ]] || {
        printf 'current_profile_g4_hard_deadline_test=FAIL final-visible\n'; exit 1;
    }
    ...
    [[ ! -e $partial/TIMING-DONE.STAMP && ! -e $publishing/TIMING-DONE.STAMP &&
       -z $(/usr/bin/find "$failed_tree" -xdev -perm /0222 -print -quit) ]] || {
        printf 'current_profile_g4_hard_deadline_test=FAIL partial-authoritative\n'; exit 1;
    }
    /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
    /usr/bin/rm -rf "$root"
    root=$(/usr/bin/mktemp -d)
    ...
    (( rc != 0 )) || { printf 'current_profile_g4_hard_deadline_test=FAIL late-accepted\n'; exit 1; }
    [[ ! -e $final && -f $late/REJECTED-TIMING-DONE.STAMP && ! -e $late/TIMING-DONE.STAMP ]] || {
        /usr/bin/cat "$late_error" >&2
        printf 'current_profile_g4_hard_deadline_test=FAIL late-not-quarantined\n'; exit 1;
    }
    ...
    [[ -f $late/REJECTED-TIMING-DONE.STAMP && ! -e $late/TIMING-DONE.STAMP &&
       -z $(/usr/bin/find "$late" -xdev -perm /0222 -print -quit) ]] || {
        printf 'current_profile_g4_hard_deadline_test=FAIL late-authoritative\n'; exit 1;
    }
    /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
    /usr/bin/rm -rf "$root"
    root=$(/usr/bin/mktemp -d)
    ...
    [[ ! -e $final && -f $late/REJECTED-TIMING-DONE.STAMP &&
       ...
       -z $(/usr/bin/find "$late" -xdev -perm /0222 -print -quit) ]] || {
        printf 'current_profile_g4_hard_deadline_test=FAIL fallback-quarantine\n'; exit 1;
    }
    /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
    /usr/bin/rm -rf "$root"
    root=$(/usr/bin/mktemp -d)
    ...
    if (( rc == 0 )) || [[ -e $final || ! -f $late/TIMING-DONE.STAMP ]] ||
       [[ $(<"$late/REJECTED-TIMING-DONE.STAMP") != collision-sentinel ]]; then
        printf 'current_profile_g4_hard_deadline_test=FAIL no-replace-collision\n'; exit 1
    fi
    /usr/bin/chmod -R u+w "$root" 2>/dev/null || true
    /usr/bin/rm -rf "$root"
    printf 'current_profile_g4_hard_deadline_test=PASS\n'
}
